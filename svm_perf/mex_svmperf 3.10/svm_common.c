...
void write_model(mxArray *mxStruct[], MODEL *model) {
	...
	pointer=mxCreateString(model->kernel_parm.custom); // this is a string
	...
	// Store the support vectors (always in a sparse matrix) and alpha*y (in a full matrix)
    mwSize nzmax=0; // number of non-cero elements
	mwSize n=0; //model->totwords-1; // columns of the support vector matrix
	mwSize m=0; // rows of the support vector matrix: will be computed with the following
	for(i=1;i<model->sv_num;i++) {
		for(v=model->supvec[i]->fvec;v;v=v->next) {
			bool at_least_one=false;
			for (j=0; (v->words[j]).wnum; j++) {
				if (n<(v->words[j]).wnum)
					n=(v->words[j]).wnum; // at the end, n will be the highest feature index
	...
	} // now we know the value of 'm' and 'n'
		...
		for(i=1;i<model->sv_num;i++) {
			for(v=model->supvec[i]->fvec;v;v=v->next) {
				...
				while (((v->words[j]).wnum > 0) && ((v->words[j]).wnum-1<full_j))
					j++;
				// we found a value to be stored (if not zero), 
				// or we finished traversing the row
				
				// Convert (full_i,(v->words[j]).wnum - 1) in a sparse position!!
				if (((v->words[j]).wnum > 0)
					&& ((v->words[j]).wnum-1==full_j)
					&& ((double)(v->words[j]).weight != 0)) {
					// this would be a (full_i,full_j) position in a full matrix
	...
	jcs[full_j]=k; // that is, jcs[model->totwords]= number of nonzero elements;
	
	mxSetField(model_st,0,"supvec",pointer); // store sparse matrix in structure
...
MODEL *read_model(const mxArray *model_st) {
	long i;
	char *comment="\0"; // no comments in matlab matrices
	WORD *words;
	MODEL *model;
	...
	model->sv_num=rows+1; // TJ seems to leave supvec[0] and alphas[0] unused
	...
	model->index=NULL;
	model->lin_weights=NULL;
	// let's load the support vector matrix	
	for(i=1;i<model->sv_num;i++) {
		parse_mxData(i-1, // OL (yeah, me) doesn't leave the first row free!! 
...
{
	// check if data comes in full or sparse matrix
	...
	long queryid=0,slackid=0; // fixed, cannot be specified in matlab
	long max_words_doc;
	double doc_label, costfactor=1; // fixed, cannot be specified in matlab
	...
	mwSize rows=size[0]; // number of rows
	mwSize cols=size[1]; // number of columns
	max_docs=rows+2; // Joachims dixit
	max_words_doc=cols+2; // Joachims dixit
	
	// Reserve memory
	(*docs) = (DOC **)my_malloc(sizeof(DOC *)*max_docs);    /* feature vectors */
	(*label) = (double *)my_malloc(sizeof(double)*max_docs); /* target values */
	words = (WORD *)my_malloc(sizeof(WORD)*(max_words_doc+10)); // +10 is Joachims code
	if(verbosity>=1) {
		printf("Reading examples into MEX routines...");
	}
	
	for (dnum=0;dnum<rows;dnum++) {
		// Read dnum row of data
		parse_mxData(dnum,mxData,mxLabels,&doc_label,words);
		
		// fill the corresponding label
		if (doc_label>0) dpos++;
		if (doc_label<0) dneg++;
...
void parse_mxData(int dnum, const mxArray *mxData, const mxArray *mxLabels, double *doc_label, WORD * words) {
	...
	double *labels=mxGetPr(mxLabels); // labels are here
	// fill in the label
	(*doc_label)=labels[dnum];
	// ... and the input data
	double *data=mxGetPr(mxData); // input data are here (without labels)
	bool sparse_data=mxIsSparse(mxData);
	if (sparse_data) { // SPARSE MATRIX
		...
		for (col=0;col<cols;col++) {
			...
			if (a!=b) {
				// there are data in this column, but...
				for (row=a;row<b;row++) {
					// ...is it in the row we are looking for?
					if (ir[row]==dnum) {
						// yes, it is!!
		...
		{
			int index = wordnum*rows + dnum; // data(dnum,wordnum)
